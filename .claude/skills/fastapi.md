...
In case of doubt, or by default, use regular `def` functions — they run in a threadpool and don't block the event loop.
...
Don't mix HTTP operations in a single function — one function per HTTP operation separates concerns and organizes the code.
